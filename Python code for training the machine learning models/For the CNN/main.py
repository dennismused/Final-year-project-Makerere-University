...
metadata = pd.read_csv('E:/Users/DENNIS/PycharmProjects/pythonProject/FYP_Project_CNN/Prepared beehive dataset/metadata.csv')
print(metadata.head())
...
def features_extractor(file):
    ...
    # Reshaping extracted feature from 1D to 2D as required through input shape of our model.
    mfccs_scaled_features = np.reshape(mfccs_scaled_features, (40, 1))
...
extracted_features_df.to_pickle('E:/Users/DENNIS/PycharmProjects/pythonProject/FYP_Project_CNN/Prepared beehive dataset/Normalised_features_CNN.pkl')


# Loading the created dataframe of the extracted features for use
extracted_features_df = pd.read_pickle('E:/Users/DENNIS/PycharmProjects/pythonProject/FYP_Project_CNN/Prepared beehive dataset/Normalised_features_CNN.pkl')

# Splitting the dataset into independent and dependent dataset and converting them into a numpy array
...
# Adding an extra dimension for number of channels (1) to the training data so that it matches the dimension expected by the CNN as input
...
model = Sequential()
# first layer - CNN layer with 16 filters and kernel size of (2x1)
model.add(Conv2D(16,(2,1),input_shape=(40,1,1)))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(1, 1)))
model.add(Dropout(0.5))
# second layer - CNN layer with 32 filters and kernel size of (3x3)
model.add(Conv2D(32,(3,1)))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(3, 1)))
model.add(Dropout(0.5))
# third layer - Flatten layer to convert output into 1D array for input into dense layer
model.add(Flatten())
model.add(Dense(100))
model.add(Activation('relu'))
model.add(Dropout(0.5))
# final layer - Dense layer with number of neurons equal to number of labels
...
checkpointer = ModelCheckpoint(filepath='E:/Users/DENNIS/PycharmProjects/pythonProject/FYP_Project_CNN/Trained CNN model/Normalised_FYP_Beehivemodel_CNN.hdf5', verbose=1, save_best_only=True)
start = datetime.now()
history = model.fit(X_train, y_train, batch_size=num_batch_size, epochs=num_epochs, validation_data=(X_valid, y_valid), callbacks=[checkpointer], verbose=1)  # Adding the callbacks to the fit method enables saving the model
...
model = load_model('Trained CNN model/FYP_Beehivemodel_CNN.hdf5')
...
for i in NoQB_files:
    ...
    mfccs_features = librosa.feature.mfcc(y=audio, sr=sample_rate, n_mfcc=40)
    mfccs_scaled_features = np.mean(mfccs_features.T, axis=0)
    # Reshape MFCC feature to match the expected input shape of the CNN model
    mfccs_scaled_features = np.expand_dims(mfccs_scaled_features, axis=-1)
